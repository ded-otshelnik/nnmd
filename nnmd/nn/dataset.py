...
class TrainAtomicDataset(Dataset):
    def __init__(self, sf_data: dict[str, torch.Tensor],
                energies: torch.Tensor, forces: torch.Tensor,
                symm_func_params: dict) -> None:
...
def make_atomic_dataset(dataset: list, 
                        symm_func_params: dict[str, dict[str, list[float | int]]]| str, **kwargs) -> TrainAtomicDataset:
    ...
    cartesians = {spec: torch.tensor(np.array([data[spec]['positions'] for data in dataset]), dtype = torch.float32, device = 'cuda')
    ...
    for spec in cartesians.keys():
        if 'saved' in kwargs and kwargs['saved'] == True:
            ...
            g_spec, dg_spec = calculate_sf(cartesians[spec].cpu(), cell.cpu(), symm_func_params[spec])
