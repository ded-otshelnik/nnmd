...
traj_file = "input/lennard_jones_trimer.traj"
d = 0.96
a = d / np.sqrt(2)
...
if plot:
    ...
    plot = [(i, frame.get_distance(0, 2)) for i, frame in enumerate(traj)]
    ...
    plt.xlabel("Distance (Å)")
    plt.ylabel("Time Step")
    plt.title("Distance 0 to 2, Lennard-Jones Trimer")
    plt.grid()
    plt.show()

    plot = [(i, frame.get_distance(2, 1)) for i, frame in enumerate(traj)]
    ...
    plt.xlabel("Distance (Å)")
    plt.ylabel("Time Step")
    plt.title("Distance 2 to 1, Lennard-Jones Trimer")
    plt.grid()
    plt.show()