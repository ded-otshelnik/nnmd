...
params_parser = argparse.ArgumentParser(
    description="Script for MD simulation with neural network"
)
...
cartesians = torch.as_tensor(
    np.load("cartesians_actual.npy.npz")["cartesians"], dtype=dtype
)
forces = torch.as_tensor(np.load("forces_actual.npy.npz")["forces"], dtype=dtype)

start = int(0.8 * len(cartesians))
v_initial = torch.as_tensor(
    np.load("velocities_actual.npy.npz")["velocities"][start], dtype=dtype
)
...
nn.config(
    ...
    n_atoms=n_atoms,
    load_models=True,
    path="../li/checkpoint.pt",
)

md_system = MDSimulation(
    N_atoms=n_atoms,
    cartesians=cartesians_initial,
    nn=nn,
    mass=m_atom,
    rVan=rVan,
    symm_func_params=symm_func_params,
    L=h,
    T=T,
    dt=dt,
    h=h,
...
np.savez(
    "cartesians_history.npy.npz", x=data[:, :, 0], y=data[:, :, 1], z=data[:, :, 2]
)
np.savez("forces_history.npy.npz", forces=np.array(md_system.forces_history))
np.savez(
    "velocities_history.npy.npz", velocities=np.array(md_system.velocities_history)
)
